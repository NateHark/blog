...
server: # runs the local hugo webserver
	docker run --rm --user $(shell id -u):$(shell id -u) -v $(CWD):/data -p 0.0.0.0:$(PORT):$(PORT) $(BLOG_CONTAINER) -w --buildDrafts --buildFuture --bind 0.0.0.0 --baseURL $(HOSTNAME):$(PORT) server

site: # builds the static website
	docker run --rm --user $(shell id -u):$(shell id -u) -v $(CWD):/data -p 0.0.0.0:$(PORT):$(PORT) $(BLOG_CONTAINER)
